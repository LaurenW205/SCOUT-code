...
def initCam(picam, res): # connect video stream 
    h, w = None, None
    if res == 480:
        config = picam.create_video_configuration({"format" : "BGR888", "size": (640, 480)}, controls={"FrameRate": 30.0})
        picam.configure(config)
        w = 640
        h = 480
    elif res == 720:
        config = picam.create_video_configuration({"format" : "BGR888", "size": (1280, 720)}, controls={"FrameRate": 30.0})
        picam.configure(config)
        w = 1280
        h = 720
    elif res == 1080:
        config = picam.create_video_configuration({"format" : "BGR888", "size": (1920, 1080)}, controls={"FrameRate": 30.0})
        picam.configure(config)
        w = 1920
        h = 1080
...
videoFile = cv2.VideoWriter('raw.avi', fourcc, 10, (frame_W, frame_H))
...
while True:
    ...
    filtered = filterYRB(frame, 70, 165, 20) 
